...
// หน้านัดหมายหลัก
// หน้านัดหมายหลัก - แบบง่ายสุด
router.get('/dentist_appointments', allowRoles('dentist'), (req, res) => {
  ...
  // 1. หา dentist_id จากตาราง dentists ก่อน
  const findDentistSql = `SELECT id FROM dentists WHERE user_id = ?`;

  db.get(findDentistSql, [userId], (err, dentist) => {
    if (err) {
      console.error('Error finding dentist:', err);
      return res.status(500).send('Database error: ' + err.message);
    }

    if (!dentist) {
      console.error('Dentist not found for user ID:', userId);
      return res.status(404).send('Dentist profile not found');
    }

    const dentistId = dentist.id;
    console.log('Found Dentist ID:', dentistId);

    // 2. ดึงนัดหมายด้วย dentist_id ที่ถูกต้อง
    const casesSql = `
    SELECT 
        a.id,
        a.slot_text,
    ...
    JOIN dental_units du ON a.unit_id = du.id
    WHERE a.dentist_id = ?
    ORDER BY a.date DESC, a.slot_text
    LIMIT 20
`;

    db.all(casesSql, [dentistId], (err, cases) => {
      if (err) {
      ...
      res.render('dentists/dentist_appointments', {
        cases: cases || [],
        date: date,
        user: req.user,
        userRole: req.user.role,
...
// Route ทดสอบ
router.get('/test-appointments', allowRoles('dentist'), (req, res) => {
    res.render('dentists/dentist_appointments', {
        cases: [
            {
                id: 1,
                slot_text: '10:00-11:00',
                status: 'CONFIRMED',
                treatment: 'ทดสอบการรักษา',
                unit_name: 'ห้อง 1',
                patient_name: 'ผู้ป่วยทดสอบ',
                clinic_number: 'CN0001',
                patient_id: 1
            }
        ],
        date: '2024-01-01',
